...
function changeHair(ele) {
    ...
    var hairTopPositionTop = 50; // 点击头发后 , 在人头上面 的初始位置
    ...
    fabric.Image.fromURL(img.getAttribute('src'), function (tmpImg) {
        ...
        hairCanvas.top = hairTopPositionTop;
        hairCanvas.left = 90;
        console.log(hairCanvas);
        hairCanvas.id = 'hair';
        hairCanvas.cornerSize = 30
        hairCanvas.cornerColor = "#F65445"
        // hairCanvas.hasBorders = false
        hairCanvas.cornerStyle = "circle"
        hairCanvas.padding = 15
        canvas.add(hairCanvas);
        hairCanvas.moveTo(100);
        hairCanvas.centerH();
        canvas.renderAll();
